...
export function SpinningWheel() {
  ...
  // Check URL for wheel parameter on initial load
  useEffect(() => {
    const wheelParam = searchParams.get("wheel");

    if (wheelParam) {
      try {
        // Check if it's a saved wheel from localStorage first
        const loadSavedWheel = () => {
          const savedWheels = getAllSavedWheels();

          // Find saved wheel by slug
        ...
        // Then check if it's a predefined wheel
        const loadPredefinedWheelFromUrl = async () => {
          try {
        ...
        // Try to load the wheel (first check saved, then predefined)
        (async () => {
          const isSaved = loadSavedWheel();
          if (!isSaved) {
            const isPredefined = await loadPredefinedWheelFromUrl();
            if (!isPredefined) {
        ...
        console.error("Error loading wheel from URL parameter:", error);
      }
    }
  }, [searchParams]);
